...
$\alpha\in]0,1[$ un paramètre, $s\in]0,1[$ le taux
...
\textbf{\textsc{Exercice II.}} On reprend le même modèle que dans
l'exercice I, mais avec la fonction de production suivante :
\[
Y(t) = K(t)^\alpha L(t)^{1-\alpha} + AK
\]
où $A$ est un paramètre strictement positif. \question La fonction de
production est-elle une fonction de production néoclassique~?
Pourquoi~? \question Déterminer la loi d'évolution du stock de capital
par tête. \question Déterminer une condition sur $s$ qui assure
l'existence d'un unique état stationnaire non trivial pour le stock de
capital par tête. \question Que se passe-t-il si l'état stationnaire
non trivial n'existe pas~? \question Comment expliquer ce résultat~?
\question Quel est alors le taux de croissance à long terme du stock
